...
The ICAT project includes a number of components, the principal ones being ICAT, IDS \(ICAT Data Service\), TopCat, IJP \(ICAT Job Portal\) and the ICAT manager. Each components has its own GitHub repository within an icat organisation \([https://github.com/icatproject](https://github.com/icatproject%29\) \) containing the source code, a small Wiki and the Isuue tracker List for that component.
